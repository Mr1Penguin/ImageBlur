...
inline float getPixelRed(const CLPixelType p) { return (float)p.x; }
inline float getPixelGreen(const CLPixelType p) { return (float)p.y; }
inline float getPixelBlue(const CLPixelType p) { return (float)p.z; }
inline float getPixelAlpha(const CLPixelType  p, const unsigned int number_channels) {
	CLQuantum val = 0;
...
inline void ReadChannels(const CLPixelType p, const unsigned int number_channels, const ChannelType channel, float *red, float *green, float *blue, float *alpha)
{
	if ((channel & RedChannel) != 0)
		*red = getPixelRed(p);
	if (number_channels > 2)
...
inline float4 ReadFloat4Private(const CLPixelType p, const unsigned int number_channels, 
	...
	write_only image2d_t tempImage)
{
	...
	for (int i = get_local_id(0); i < loadSize; i = i + get_local_size(0))
	{
		...
		CLPixelType pixel = as_float4(intel_sub_group_block_read4(((const __global uint *)image) + cx * 4 + y * imageColumns * 4));
		temp[i] = ReadFloat4Private(pixel, number_channels, channel);
	}
